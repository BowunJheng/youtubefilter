...
chrome.webNavigation.onCompleted.addListener(function(details) {
    chrome.tabs.getSelected(null,function(tab) {
        chrome.tabs.executeScript(details.tabId, {code: 'blackListFilter("'+tab.title+'",1);'});
    });
...
chrome.webNavigation.onTabReplaced.addListener(function(details) {
    chrome.tabs.getSelected(null,function(tab) {
        chrome.tabs.executeScript(details.tabId, {code: 'blackListFilter("'+tab.title+'",2);'});
    });
}, { url: [{ hostContains: '.youtube.' }], });
*/
var lastTitle="";
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('button').addEventListener('click', function(event){modidyBlackList(ACTION.ADD, event);});
    document.getElementById('list').addEventListener('click', getOneRecord);
    document.getElementById('delbtn').addEventListener('click', function(event){modidyBlackList(ACTION.DEL, event);});
});
